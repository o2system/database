...
class Connection extends AbstractConnection
{
     ...
     * DELETE hack flag
     *
     * Whether to use the SQLite "delete hack" which allows the number
    ...
    protected function platformConnectHandler( Config $config )
    {
        ...
        $flags = ( $config->compress === true )
            ? SQLiteI_CLIENT_COMPRESS
            : 0;
        $this->handle = SQLitei_init();
        //$this->handle->autocommit( ( $this->transactionEnabled ? true : false ) );

        $this->handle->options( SQLiteI_OPT_CONNECT_TIMEOUT, 10 );

        if ( isset( $config->strictOn ) ) {
            if ( $config->strictOn ) {
                $this->handle->options(
                    SQLiteI_INIT_COMMAND,
            ...
            if ( ! empty( $ssl ) ) {
                if ( isset( $config->encrypt[ 'ssl_verify' ] ) ) {
                    if ( $config->encrypt[ 'ssl_verify' ] ) {
                        defined( 'SQLiteI_OPT_SSL_VERIFY_SERVER_CERT' )
                        && $this->handle->options( SQLiteI_OPT_SSL_VERIFY_SERVER_CERT, true );
                    }
                    // Apparently (when it exists), setting SQLiteI_OPT_SSL_VERIFY_SERVER_CERT
                    // to FALSE didn't do anything, so PHP 5.6.16 introduced yet another
                    // constant ...
                    //
                    // https://secure.php.net/ChangeLog-5.php#5.6.16
                    // https://bugs.php.net/bug.php?id=68344
                    elseif ( defined( 'SQLiteI_CLIENT_SSL_DONT_VERIFY_SERVER_CERT' ) ) {
                        $this->handle->options( SQLiteI_CLIENT_SSL_DONT_VERIFY_SERVER_CERT, true );
                    }
                }

                $flags |= SQLiteI_CLIENT_SSL;
                $this->handle->ssl_set(
                    isset( $ssl[ 'key' ] )
                        ? $ssl[ 'key' ]
                        : null,
        ...
        if ( $this->handle->real_connect(
            ...
            // Prior to version 5.7.3, SQLite silently downgrades to an unencrypted connection if SSL setup fails
            if (
                ( $flags & SQLiteI_CLIENT_SSL )
                ...
                $this->handle->close();
                // 'SQLitei was configured for an SSL connection, but got an unencrypted connection instead!';
    ...
    protected function platformTransactionBeginHandler()
    {
        ...
        return $this->handle->begin_transaction( SQLiteI_TRANS_START_READ_WRITE );
    }
    ...
    {
        // SQLitei_affected_rows() returns 0 for "DELETE FROM TABLE" queries. This hack
    ...
    protected function platformQueryHandler( Query &$query )
    {
        ...
        if ( false !== ( $result = $this->handle->query( $query->getFinalStatement() ) ) ) {
            $rows = $result->fetch_all( SQLiteI_ASSOC );
        } else {
